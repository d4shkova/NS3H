...
/**
 * SFTP over a connection of its own (§ phase 12) — no shell, no terminal, no session log.
 *
 * This is the same `SshConnection` a terminal session uses, opened with `shell: false`,
 * so a transfer target gets the algorithm ladder, the known-hosts check and the auth
 * re-prompt without any of it being written twice.
 */
export class StandaloneSftp implements FileTransport {
  ...
  static async connect(
    ...
    // `open` resolves once the ladder is done; it reports the outcome through the
    // callbacks rather than by throwing, so the failure is captured here and turned
    // back into a rejection the pane can show.
    ...
    const connection = new SshConnection(
      sshTarget,
      {
         ...
         *
         * The consequence, and it is a real one: a device demanding a second factor
         * cannot be reached this way. Open an SSH session and transfer over that.
         */
        promptAuth: (request) => {
